...
int main() {
  ...
  rep(i, m) {
    ...
    if (d == 1) {
      to[a].emplace_back(make_pair(b, 10000LL*c - 1LL));
      to[b].emplace_back(make_pair(a, 10000LL*c - 1LL));
    } else {
      to[a].emplace_back(make_pair(b, 10000LL*c));
      to[b].emplace_back(make_pair(a, 10000LL*c));
    }
  }
  ...
  int dist = (cur[n] + 9999) / 10000;
  int t = dist * 10000 - cur[n];
